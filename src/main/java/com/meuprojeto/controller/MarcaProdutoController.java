...
import com.meuprojeto.repository.MarcaRepository;
...
public class MarcaProdutoController {
    ...
    public ResponseEntity<MarcaProduto> salvarMarca(@RequestBody @Valid MarcaProduto marcaProduto) throws ExcecaoMsgErro { /*Recebe o JSON e converte pra Objeto*/

        if (marcaProduto.getId() == null) {
            List<MarcaProduto> acessos = marcaRepository.buscarMarcaDesc(marcaProduto.getNomeDesc().toUpperCase());

            if (!acessos.isEmpty()) {
                throw new ExcecaoMsgErro("Já existe Marca com a descrição: " + marcaProduto.getNomeDesc());
            }
        }
    ...
    public ResponseEntity<MarcaProduto> obterMarca(@PathVariable("id") Long id) throws ExcecaoMsgErro {

        MarcaProduto marcaProduto = marcaRepository.findById(id).orElse(null);

        if (marcaProduto == null) {
            throw new ExcecaoMsgErro("Não encontrou a Marca com código: " + id);
        }
